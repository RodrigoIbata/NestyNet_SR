...
def run_shared_candidate_training(
    *,
    ...
    bundle = train_shared_candidate_dual_ranker(
        ...
        common_q_weight=float(common_q_weight),
        preview_value_weight=float(preview_value_weight),
        preview_regret_weight=float(preview_regret_weight),
        common_state_value_weight=float(common_state_value_weight),
        repair_state_value_weight=float(repair_state_value_weight),
        build_state_value_weight=float(build_state_value_weight),
        init_bundle=init_bundle,
    )
    out_path = Path(output_path)
    save_repair_critic_bundle(bundle, out_path)
    pred_bundle = load_repair_critic_bundle(out_path)
    sample_pred = None
    for row in rows:
    ...
    summary = {
        ...
        "shared_candidate_dual_target": str(bundle.get("shared_candidate_dual_target", "")),
...
def main() -> None:
    ...
    p.add_argument("--pairwise_weight", type=float, default=0.25)
    p.add_argument("--path_ce_weight", type=float, default=0.05)
    p.add_argument("--common_value_weight", type=float, default=0.25)
    p.add_argument("--common_regret_weight", type=float, default=1.0)
    p.add_argument("--common_q_weight", type=float, default=0.20)
    p.add_argument("--preview_value_weight", type=float, default=0.10)
    p.add_argument("--preview_regret_weight", type=float, default=0.10)
    p.add_argument("--common_state_value_weight", type=float, default=0.10)
    p.add_argument("--repair_state_value_weight", type=float, default=0.10)
    p.add_argument("--build_state_value_weight", type=float, default=0.10)
    args = p.parse_args()
    ...
    summary = run_shared_candidate_training(
        ...
        common_value_weight=float(args.common_value_weight),
        common_regret_weight=float(args.common_regret_weight),
        common_q_weight=float(args.common_q_weight),
        preview_value_weight=float(args.preview_value_weight),
        preview_regret_weight=float(args.preview_regret_weight),
        common_state_value_weight=float(args.common_state_value_weight),
        repair_state_value_weight=float(args.repair_state_value_weight),
        build_state_value_weight=float(args.build_state_value_weight),
    )
