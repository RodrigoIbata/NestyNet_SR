...
"""
Logistic-growth discovery runner for current DE-library conventions.

Stages:
1. Baseline sparse DE discovery
2. Power-template search (heuristic psi init)
3. Power-template search with LM-over-psi
"""
...
def _extract_power_param(template_params: dict) -> float | None:
...
def _find_power_term(coeff_map: dict[str, float]) -> str | None:
...
def summarize_stage(
    ...
    power_term = _find_power_term(coeff_map)
    power_coeff = float(coeff_map.get(power_term, 0.0)) if power_term is not None else None
    ...
    p_val = _extract_power_param(template_params)

    issues: list[str] = []
    critical = False

    if abs(u_coeff - (-0.5)) > 0.25:
        issues.append(f"u coeff off: expected ~-0.5, got {u_coeff:.4f}")

    if "template" in stage_name.lower():
        if power_term is None:
            critical = True
            issues.append("missing u^p template term")
        else:
            if power_coeff is not None and abs(power_coeff - 0.05) > 0.04:
                issues.append(f"u^p coeff off: expected ~0.05, got {power_coeff:.4f}")
            if p_val is None:
                issues.append("missing recovered power exponent")
            elif abs(p_val - 2.0) > 0.25:
                issues.append(f"power exponent off: expected ~2.0, got {p_val:.4f}")
...
def identifiability_screen(
    ...
    *,
    ...
    p_val = _extract_power_param(varpro_meta.get("template_params", {}) or {})
...
def _choose_model_branch(
    *,
    ...
    if linear_candidate is None and nonlinear_candidate is None:
        ...
        return "FAIL", str(details["reason"]), details
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Run logistic-growth discovery tests with modern DE-library settings",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("--datafile", type=Path, default=repo_root / "data" / "logistic_growth.csv")
    parser.add_argument(
        "--output_dir",
        type=Path,
        default=repo_root / "results" / "logistic_growth",
        help="Directory for stage artifacts",
    )
    ...
    parser.add_argument("--template_lm_epochs", type=int, default=120, help="LM epochs for psi")
    parser.add_argument(
        "--selection_bic_delta",
        type=float,
        default=2.0,
    ...
    generate_script = script_dir / "generate_logistic_growth.py"
    ...
    if args.generate:
        ok, _ = run_command(
            ...
            "GENERATING SYNTHETIC LOGISTIC-GROWTH DATA",
        )
        if not ok:
            return 1
    ...
    if not args.skip_baseline:
        ...
        if not ok:
            ...
            stage_results["baseline"] = summarize_stage(
                "baseline", coeff_map, de_meta, baseline_rms_val=None
            )
    ...
    if not args.skip_heuristic:
        ok, _ = run_command(
            ...
            "TEST 2: POWER TEMPLATE (HEURISTIC PSI)",
        )
        if not ok:
